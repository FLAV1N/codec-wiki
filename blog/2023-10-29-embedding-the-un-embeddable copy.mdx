...
- You can embed non-web compatible codecs such as [HEVC](https://codecs.wiki/docs/video/HEVC) in [MP4/MOV](https://codecs.wiki/docs/introduction/terminology#mp4--m4v), but the user must be using a compatible browser. [Thorium](https://thorium.rocks) or Safari version 13 or greater will work for HEVC playback.
...
After the discovery of AV1 embedding, experimentation brought about the discovery that *any* video codec will work as long as the user can decode/play the codec and the container/extension is an MP4, MOV, or WebM. These are all traditionally web-compatible containers. If you're interested in learning about containers, please see the [Containers](https://codecs.wiki/docs/introduction/terminology#container) section on the [Terminology](https://codecs.wiki/docs/introduction/terminology) page.

This applies to HEVC, ProRes, [xHE-AAC](https://codecs.wiki/docs/audio/AAC#xhe-aac), and other bizarre codecs that are rarely seen on the Web.
