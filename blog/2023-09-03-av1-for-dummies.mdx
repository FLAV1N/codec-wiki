...
While a lot of the information presented in this guide is still relevant and correct, the AV1 ecosystem has changed dramatically since this guide's inception which has demanded a rewrite. Please see the [AV1 for Dummies](https://codecs.wiki/blog/av1-for-dummies) blog post for more information.
:::

![Compare](/img/compare-guide.webp)

In this guide, we'll be installing Av1an for chunked encoding and infinite threading, because the current state of AV1 encoders, except for [SVT-AV1](https://codecs.wiki/docs/encoders/SVT-AV1), unfortunately lacks threading and will only use very low amount of cores, which hampers speeds. The only caveat to this approach is **RAM consumption**, encoding 2160p (4K) with [aomenc](https://codecs.wiki/docs/encoders/aomenc) with 4 workers could take upwards of **16GB** of RAM! So do keep this in mind.
...
The easiest way to encode with WSL2 is to use [rAV1ator CLI](https://codecs.wiki/docs/utilities/rav1ator-cli), an interactive TUI for [Av1an](https://codecs.wiki/docs/utilities/av1an). An ArchWSL2 installation tutorial is provided [here](https://codecs.wiki/docs/utilities/rav1ator-cli#windows).
...
:::info
If you opt to compile aomenc yourself, you can view the instructions on how to do that [here](https://codecs.wiki/docs/encoders/aomenc/#installation).
:::
...
- Install [Aviator](https://github.com/gianni-rosato/aviator) ([SVT-AV1](https://codecs.wiki/docs/encoders/SVT-AV1) + [FFmpeg](https://codecs.wiki/docs/utilities/FFmpeg)) or [rAV1ator](https://giannirosato.com/blog/post/aviator-1/) basically same thing but [Av1an](https://codecs.wiki/docs/utilities/av1an.mdx) + [rav1e](https://codecs.wiki/docs/encoders/rav1e). Both are only available as [Flatpaks](https://beta.flathub.org/apps/net.natesales.Aviator). Keep in mind Aviator ships with **SVT-AV1** and rAV1ator with **rav1e** instead of aomenc/AOM-AV1, which I will not be covering here.

### The TUI Way

*(Recommended)*

- Install [rav1ator-cli](https://codecs.wiki/docs/utilities/rav1ator-cli), a TUI for using Av1an meant to be easy to use. Much more flexible than the GUI options & can work with a number of encoders. See [this page](https://codecs.wiki/docs/utilities/rav1ator-cli/#installation) for more info. Can be easily used on any distro.
